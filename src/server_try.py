...
if __name__ == "__main__":
    try:
        ...
        nextIndex = [0, 0, 0, 0, 0]  ## the index is not fixed????
        matchIndex = [1, 1, 1, 1,
                      1]  ## the value is matchIndex 2,3,1,2,3, for example, if the leader is 0, then it means fellower 1 have replicated 3 entries
        ...
        period_election = 1  ##s
        reset_timer = 0  ###????
        follower_time_out = threading.Event()
        ##
        ##
...
class _thread_leader_fellower (threading.Thread):
    ...
    def follower_action(self):
        ...
        threading.Timer(period_election, self.follower_timeout,
                        (follower, leader_term)).start()
        follower_time_out.wait()
        while flag_follower:
            time.sleep(0.01)
