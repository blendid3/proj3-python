...
class _thread_leader_fellower (threading.Thread):
    ...
    def candidate_action(self):
        global followers, getvotes,voteGranted,candidate_exit,flag_leader,flag_candidate,candidate_time_out
        threading.Thread(period_heartbeat,self.candidate_timeout)
    ...
    def request_vote_from_follower(self,follower):
        ...
        if flag_follower==1 or flag_leader==1:
            return
        ...
        if flag==True:
            ...
            getvotes+=1
            self.candidate_result=1
        if flag==-1:## crash
            threading.Timer(period_heartbeat,self.request_vote_from_follower,(follower,result)).start()
        if flag==False:
    ...
    def commit_increase(self):
        global flag_leader,nextid,match_index,flag_follower,id,commitIndex
        get_agree=0
        if flag_leader==0:
            return
        else:
            for i in range(maxnum):
                if nextid[i]>commitIndex+1:
                    get_agree+=1
    ...
    def client_push(self,client_signal,follower,leader_term):
        global flag_leader,nextid,match_index,flag_follower
    ...
    def heartbeat_periodically(self,follower,leader_term):##
        global flag_leader, currentTerm,nextid,match_index,flag_follower ,log
        if flag_leader == 0:
